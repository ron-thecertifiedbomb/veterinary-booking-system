...
export default function Home() {
  ...
  return (
    ...
    >
      {/* ─── HEADER SECTION ─── */}
      <View className="flex-row justify-between items-center mb-8 pb-4 border-b border-zinc-100">
        <View>
      ...
      <View className="flex-row items-center space-x-2 px-4 py-3 bg-white border border-zinc-100 rounded-2xl mb-6 gap-2">
        <View className={`h-1.5 w-1.5 rounded-full ${loading ? 'bg-zinc-400' : 'bg-black'}`} />
        <Text className="text-[9px] font-black tracking-[0.15em] uppercase text-zinc-500">
      ...
      <View className="flex-row flex-wrap justify-between">
        {cards.map((card, index) => {
          ...
          return (
            <View
              key={index}
              className="w-[48%] p-5 bg-white border border-zinc-100 rounded-3xl mb-4"
            >
              {/* Card Label and Mini Icon Row */}
              <View className="flex-row justify-between items-center mb-5">
                <Text className="text-[9px] font-black tracking-[0.15em] uppercase text-zinc-400 max-w-[70%]">
                  {card.label}
                </Text>
                <View className="p-1.5 rounded-xl border border-zinc-100 bg-white">
              ...
              <Text className="text-[9px] font-black tracking-[0.15em] uppercase text-zinc-400 max-w-[70%]">
               Total
                ...
                <View className={`px-2 py-0.5'}`}>
                  <Text className={`text-[14px] font-black tracking-wider`}>
                  {card.value}
                  </Text>
                </View>
              </View>
